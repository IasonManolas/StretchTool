...
bool Vector3D::isVertexVisible(Vector3D cameraPos, Vector3D normal)
{
	Vector3D vertexPos(x, y, z);
	Vector3D posToCam = cameraPos - vertexPos;
	posToCam = posToCam.Normalize(); //direction
	float dotPr = Dot(normal, posToCam);
	float angle = acos(dotPr);
	if (abs(angle) < 3.1415927 / 2.0) return true;
	return false;
